...
We argue that the as-needed strategy that developers frequently use to write code is incompatible with the systematic strategy that is necessary to satisfy version control best practices, resulting in non-ideal commit histories. 
...
\caption{A mockup of Code Bubbles, extended with Commit Bubble elements. (A), (B) and (E) are code bubbles, (C) is a \emph{task context}, that is, a panel of working sets, (D) is an expanded commit bubble, consisting of three squashed commit bubbles}
\label{fig:eclipse}
\end{figure*}
...
A potential solution is to untangle commit histories into systematic histories using history revision operations offered by modern version control systems\footnote{Robertson calls this ``sausage making'' -- ``The process of developing software, 
...
However, in practice, this procedure is intricate and difficult to perform correctly~\cite{SausageMaking}.
...
developers are able are ordinarily able to successfully create systematic histories, and that revision is an exceptional situation. However, research indicates that exceptions are normal in work processes, and tools should support handling 
...
The groundwork, Code Bubbles, through which we implement our idea, uses a metaphor of light-weight editable code fragments, which are displayed on a canvas in the editor as \emph{bubbles} (Figure~\ref{fig:eclipse}). 
...
We assert one reason both of these activities (coding and debugging) are well-suited for this metaphor because the interaction aligns with the as-needed strategy that developers frequently use. 
...
Anthony is a software developer who has been tasked with adding a new feature to the Eclipse JDT 4.0 branch\footnote{This example is based on the \texttt{Main.java} file located at \url{http://download.eclipse.org/eclipse/downloads/drops4/R-4.4.1-201409250400/#JDTCORE}}. %TODO
...
As he examines the method, he notices a particularly complicated piece of logic within one of the loops, which he decides to refactor to \texttt{isParentOf} using a combination of the extract method feature of his editor and some manual cleanup~(Figure \ref{fig:eclipse}B).
...
These three commit bubbles are all part of the same change, so he combines them into one larger commit bubble
...
He squashes these two commit bubbles into one~(Figure \ref{fig:cross-branch}A).
This bug exists in the 3.9.1 branch as well, which he verifies instantiating the corresponding code bubble (Figure \ref{fig:eclipse}E).
%He would like to copy the two commits dealing with the bugfix from the 4.0 branch to the 3.9.1 branch.

\begin{figure}
\centering
\includegraphics[height=1.9in]{presquash-history}
\caption{Commit bubbles generated automatically while developing code.  A solid outline indicates the commit bubble has been stored to public history, dashed outlines indicate commits that can be reordered or deleted}
...
\caption{Copying commit bubbles across working sets.}
\label{fig:cross-branch}
\end{figure}
In a traditional version control system, Anthony would not be able to easily transfer his commits across the two branches;
...
More sophisticated approaches exist, but they are even more burdensome and error-prone because it relies on complex and non-obvious git commands~\cite{SausageMaking}.
...
First, Anthony might forget details of his original task, for example, that \texttt{shouldIgnoreOptionalProblems} is in a broken state.
Second, even if he does remember, he now needs to recreate his working set which duplicates work he had already done.

\begin{figure}
\centering
\includegraphics[width=2.2in]{cross-branch-diff}
\caption{An inline diff visualization of a squashed commit bubble. The diff is presented on an already-visible code bubble present within in the current task context.}
\label{fig:diff}
\end{figure}
...
% \subsection{Other Interactions with Commit Bubbles}

% Our presented narrative did not capture all of the envisioned interactions that we think are useful for history revision tasks. For example:

% \begin{itemize}
% \item \textbf{Split bubble}. To separate a squashed commit, you can drag the commit of interest outside of the bubble.
% \item \textbf{Show link between code and commit bubbles}. By clicking on a commit, the relevant code bubbles associated with that commit will visually highlighted, if it is available. Given a code bubble, a similar inverse operation is also possible.
% \item \textbf{Convert commit bubble to code bubble.} The code within a commit bubble can be transformed into an editable code bubble.
% \item \textbf{Create new branches}. A new branch can be created by right-clicking on the canvas or an existing commit bubble. An empty branch label is created, and the label description can be edited by double-clicking on the label.
% \item \textbf{Untracked files} By default, untracked files and their commit bubbles are appended to a temporary unnamed branch.
% \item \textbf{Delete file from history}. The deletion of file will automatically propagate commits. Optionally, the developer can also specify that the deletion only apply to the current commit. Such an operation is useful when a sensitive file has accidentally been committed to the repository.~\footnote{\url{https://help.github.com/articles/remove-sensitive-data/}}
% \end{itemize}
...
the operations required to perform the change were significantly misaligned than their verbal description of the task, and that there was no intermediate feedback by the tool when performing the transformations (such as through the Git \emph{rebase} history revision command). 
...
Importantly, we tacitly assumed that the future of IDEs will use the bubbles metaphor; further user studies are needed to validate the appropriateness of this metaphor as a general development environment for different tasks.
